...
class TCPServer(gameActor: ActorRef) extends Actor {
  ...
  def handleMessageFromWebServer(messageString:String):Unit = {
    ...
    messageType match {
      case "connected" => gameActor ! newsprm(username)
      case "disconnected" => gameActor ! removesprm(username)
      case "move" =>
        val x = (message \ "sprmlft").as[Double]
        val y = (message \ "sprmrght").as[Double]
        gameActor ! movesprm(username, x, y)
